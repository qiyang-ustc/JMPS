...
S = 2
res = 0.0
mps = MPS(L,3,31,FloatType=Float64,ArrayType=CuArray)
for i = 1:1:L
...
mps2 = MPS(L,3,15,FloatType=Float64,ArrayType=CuArray)
mpo = MPO(L,3,5,FloatType=Float64,ArrayType=CuArray)
for i = 1:1:L
...
mps2 = MPS(L,3,15,FloatType=ComplexF64,ArrayType=CuArray)
mpo = MPO(L,3,5,FloatType=ComplexF64,ArrayType=CuArray)
for i = 1:1:L
    mps2[i] = rand(mps2.bdim[i-1],mps2.S,mps2.bdim[i])
    mpo[i] = rand(mpo.bdim[i-1],mpo.S,mpo.bdim[i],mpo.S)
end
